...
// ── §1 — der Umfang ist eingefroren ────────────────────────────────────────
{
  ...
  ok(probes === 1 && reads === 31 && catalogue === 13 && mutations === 40,
    `SCOPE 1 Probe + 18 Auskünfte + 13 typisierte Auskünfte + 40 Buchungen (${probes}/${reads}/${catalogue}/${mutations})`);
  ...
  ok(rustOps.length === 72, `SCOPE und Rust lässt genau dieselben 64 Namen durch (${rustOps.length})`);
}
