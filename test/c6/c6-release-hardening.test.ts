...
// ── §1 — der Umfang ist eingefroren ────────────────────────────────────────
{
  ...
  ok(probes === 1 && reads === 66 && catalogue === 48 && mutations === 41,
    `SCOPE 1 Probe + 18 Auskünfte + 48 typisierte Auskünfte + 41 Buchungen (R5F.1) (${probes}/${reads}/${catalogue}/${mutations})`);
  ...
  ok(allow.length === 41, `SCOPE die Erlaubnisliste nennt genau 40 Buchungen (${allow.length})`);
  ...
  ok(rustOps.length === 108, `SCOPE und Rust lässt genau dieselben 108 Namen durch (${rustOps.length})`);
}
