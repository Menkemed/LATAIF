...
// ── §1 — der Umfang ist eingefroren ────────────────────────────────────────
{
  ...
  ok(probes === 1 && reads === 65 && catalogue === 47 && mutations === 40,
    `SCOPE 1 Probe + 18 Auskünfte + 47 typisierte Auskünfte + 40 Buchungen (${probes}/${reads}/${catalogue}/${mutations})`);
  ...
  ok(rustOps.length === 106, `SCOPE und Rust lässt genau dieselben 106 Namen durch (${rustOps.length})`);
}
