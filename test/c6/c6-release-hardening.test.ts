...
// ── §1 — der Umfang ist eingefroren ────────────────────────────────────────
{
  ...
  ok(probes === 1 && reads === 23 && catalogue === 5 && mutations === 40,
    `SCOPE 1 Probe + 18 Auskünfte + 5 typisierte Auskünfte + 40 Buchungen (${probes}/${reads}/${catalogue}/${mutations})`);
  ...
  ok(rustOps.length === 64, `SCOPE und Rust lässt genau dieselben 64 Namen durch (${rustOps.length})`);
}
