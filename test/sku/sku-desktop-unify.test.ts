...
// ── a peek costs nothing ────────────────────────────────────────────────────
{
  ...
  eq(first, 'ROL-WCH-001', 'a fresh stem previews its first number');
...
// ── peek and allocate never disagree ────────────────────────────────────────
{
  ...
  createProduct(raw, db, 'Rolex', WATCH, 'ROL-WCH-007');
  const peeked = peekNextSku(db, seed);
  const claimed = createProduct(raw, db, 'Rolex', WATCH);
  eq(peeked, claimed, 'they agree across a manually-typed number too');
  eq(claimed, 'ROL-WCH-008', 'and both walked past the number the operator had taken by hand');
  raw.close();
}
...
// them must count 1,2,3,4 through a single row, not two counters that each start at 1.
{
  ...
  eq(desktop(), 'ROL-WCH-001', 'desktop takes the first number of a fresh stem');
  eq(mobile(), 'ROL-WCH-002', 'the phone continues from it rather than starting over');
  eq(desktop(), 'ROL-WCH-003', 'and the desktop continues from the phone');
  eq(mobile(), 'ROL-WCH-004', 'alternating leaves no gap and no repeat');

  const rows = counters(raw);
  eq(rows.length, 1, 'ONE counter row serves both surfaces');
  eq(rows[0].stem, 'ROL-WCH-', 'keyed by the stem, not by the surface');
...
{
  const { raw, db } = fresh();
  eq(createProduct(raw, db, 'Rolex', WATCH), 'ROL-WCH-001', 'first');
  eq(createProduct(raw, db, 'Rolex', WATCH), 'ROL-WCH-002', 'second');
  const third = createProduct(raw, db, 'Rolex', WATCH);
  eq(third, 'ROL-WCH-003', 'third');
  deleteProduct(raw, third);
  eq(peekNextSku(db, seedFor('Rolex', WATCH)), 'ROL-WCH-004', 'the preview already refuses to offer the retired number');
  eq(createProduct(raw, db, 'Rolex', WATCH), 'ROL-WCH-004', 'and the next create goes past it, not back into the gap');
  raw.close();
}
...
// DESKTOP_SKU_STALE_PREVIEW_SAFE — the form looked at 001 and someone else created it first.
{
  ...
  eq(shown, 'ROL-WCH-001', 'the desktop form is showing the first number');
  ...
  eq(saved, 'ROL-WCH-002', 'the desktop save claims the NEXT number instead of the stale one');
...
// ── a typed SKU is never replaced, and never claims a number ────────────────
{
  ...
  eq(resolveSkuDurable(db, '  ROL-WCH-042  ', 'Rolex', WATCH), 'ROL-WCH-042', 'a typed value is trimmed, not renumbered');
  eq(resolveSkuDurable(db, 'null', 'Rolex', WATCH), 'ROL-WCH-001', 'the string "null" means empty — the shared emptiness rule');
  raw.close();
}
...
// out (they were never used by anything), and the manual number itself is never handed out twice.
{
  ...
  createProduct(raw, db, 'Rolex', WATCH, 'ROL-WCH-015');                // typed by hand, far ahead
  eq(createProduct(raw, db, 'Rolex', WATCH), 'ROL-WCH-009', 'the next auto-create takes 009 — a number nothing has ever held');

  for (let i = 0; i < 5; i++) createProduct(raw, db, 'Rolex', WATCH);   // 010..014
  eq(createProduct(raw, db, 'Rolex', WATCH), 'ROL-WCH-016', 'and when the walk reaches the manual number it steps over it');
  const skus = (raw.prepare(`SELECT sku FROM products ORDER BY sku`).all() as Array<{ sku: string }>).map(r => r.sku);
...
{
  const { raw, db } = fresh();
  raw.prepare(`INSERT INTO products (id, sku, brand, category_id) VALUES ('old','ROL-WCH-004','Rolex',?)`).run(WATCH);
  raw.prepare(`INSERT INTO sync_changelog (table_name, data) VALUES ('products', ?)`).run(JSON.stringify({ sku: 'ROL-WCH-009' }));
  eq(peekNextSku(db, seedFor('Rolex', WATCH)), 'ROL-WCH-010', 'the preview starts above the retired number in the changelog');
  eq(createProduct(raw, db, 'Rolex', WATCH), 'ROL-WCH-010', 'and the first claim agrees with it');
  raw.close();
}
...
{
  const { raw, db } = fresh();
  eq(createProduct(raw, db, 'Rolex', WATCH), 'ROL-WCH-001', 'Rolex watch');
  eq(createProduct(raw, db, 'Omega', WATCH), 'OME-WCH-001', 'a different brand starts its own count');
  eq(createProduct(raw, db, 'Rolex', 'cat-accessory'), 'ROL-ACC-001', 'and so does a different category');
