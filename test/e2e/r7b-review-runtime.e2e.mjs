// ════════════════════════════════════════════════════════════════════════════
// POST-PARITY R7B — Review-Nachweislauf (gezielt, nur der Primary, echte Anwendung).
// Run: node test/e2e/r7b-review-runtime.e2e.mjs
//
//   CLOSE  Reguläres Beenden (Fenster schließen → Close-Orchestrierung → AppHandle::exit) UNMITTELBAR nach
//          einer frischen Anmeldung bzw. einem frischen Seitenspeicher-Eintrag — ohne feste Wartezeit. Der
//          nächste Start ist angemeldet und findet den Eintrag. (Der R7B-Lauf beendet hart per taskkill und
//          wartet dafür fest 6,5 s; dieser Lauf zeigt, dass der reguläre Weg das nicht braucht.)
//   PP-4   Der Taktgeber in der ECHTEN Anwendung: die Uhr der Seite steht per Test-Skript kurz vor
//          Mitternacht am 30.09.; ohne Klick, ohne Neuladen, ohne Ausgabenliste entsteht der Oktober genau
//          einmal, ein weiterer Takt legt nichts nach. Nach regulärem Neustart (Uhr 02.12.) werden November
//          und Dezember nachgeholt — jeder Monat genau einmal, nichts doppelt.
//   PP-12  Fristmessung ohne Oberfläche: dieselbe Fernbuchung (/api/command) mit dem größten Dokument
//          (dreimal, wachsende Datenbank), die Inhaltsauskunft und die Erkennung an 24 MP — Zeit je Anfrage
//          am Rechner des Aufrufers gemessen (Loopback, performance.now).
//
...
const OWNER_PW = 'r7b-review-owner-' + Math.random().toString(36).slice(2);
...
const B_ID = 'user-r7b-review-b';
const B_EMAIL = 'kollege.r7b-review@lataif.com';
const B_NAME = 'PP14 Kollege B';

const RUN = join(os.tmpdir(), 'lataif-r7b-review', 'run-' + Date.now());
...
try {
  ...
  if (await exists(primary, 'input[placeholder="e.g. Al-Khalifa Luxury"]')) {
    await setVal(primary, 'input[placeholder="e.g. Al-Khalifa Luxury"]', 'R7B Review Co');
    await setVal(primary, 'input[placeholder="e.g. Main Store"]', 'R7B Review Branch');
    await clickText(primary, 'Next'); await waitFor(primary, 'input[placeholder="Full name"]');
    await setVal(primary, 'input[placeholder="Full name"]', 'R7B Review Admin A');
  ...
  // Helfer dieses Laufs
  // ══════════════════════════════════════════════════════════════════════
  const RV = { closeLogin: RV_STATE.closeLogin === true, closeProbe: false, schedulerOct: false, schedulerOnce: false, restartCatchUp: false, uploads: false, content: false, ocr: false };
  const MESS = {};
  RV_STATE = RV; MESS_STATE = MESS;
  ...
  const befehl = (op, payload, token) => api('/api/command', { op, commandId: crypto.randomUUID(), payload }, token);
  const monate = (tid) => dbQ(BIZ_DB, "SELECT substr(expense_date, 1, 7) AS m, status FROM expenses WHERE recurring_template_id = ? ORDER BY expense_date", [tid]).map((r) => r.m + (r.status === 'CANCELLED' ? '×' : ''));
  const uhrSkript = (zielMs) => `(() => { const R = Date; const OFF = ${Math.round(zielMs - Date.now())};
    class F extends R { constructor(...a) { if (a.length === 0) super(R.now() + OFF); else super(...a); } static now() { return R.now() + OFF; } }
    globalThis.Date = F; })();`;
  const seitenZeit = () => primary.ev('return new Date().getTime();').then(Number);
  /** Die Uhr der Seite stellen (Test-Skript vor jedem Seitenaufbau dieser Verbindung) und neu laden; prüft, dass sie steht. */
  async function uhrStellen(ziel) {
    await primary.send('Page.enable', {});
    const { identifier } = await primary.send('Page.addScriptToEvaluateOnNewDocument', { source: uhrSkript(ziel.getTime()) });
    await primary.ev('setTimeout(() => location.reload(), 50); return 1;');
    await sleep(6000);
    await waitFor(primary, SHELL, 90000);
    const jetzt = await seitenZeit();
    if (Math.abs(jetzt - ziel.getTime()) > 30000) throw new Error(`clock not applied: page ${new Date(jetzt).toString()} vs ${ziel.toString()}`);
    return identifier;
  }

  // ══════════════════════════════════════════════════════════════════════
  // PP-4 — der Taktgeber in der echten Anwendung
  // ══════════════════════════════════════════════════════════════════════
  let TID = '';
  {
    const login = await api('/api/auth/login', { email: OWNER_EMAIL, password: OWNER_PW });
    const token = login.j?.token;
    ok(login.status === 200 && !!token, `SETUP Anmeldung am Primary-Server (${login.status})`);
    MESS.token = token;
    const tc = await befehl('expenses.template_create', { category: 'Rent', amount: 100, paymentMethod: 'bank', payNowDefault: false, description: 'R7B Review Miete', dayOfMonth: 10, startDate: '2026-09-10' }, token);
    TID = String(tc.j?.value?.templateId || '');
    await spuelen(primary);
    const vorher = monate(TID);
    ok(tc.status === 200 && !!TID && S(vorher) === S(['2026-09']), `SETUP Dauerauftrag über den echten Fernbefehl: September angelegt (${tc.status}; ${S(vorher)})`);

    // Die Uhr der Seite: 30.09. 23:59:15 Ortszeit. Danach: kein Klick, kein Neuladen, keine Ausgabenliste.
    const ziel = new Date(2026, 8, 30, 23, 59, 15);
    const skriptId = await uhrStellen(ziel);
    const start = await seitenZeit();
    const route = await primary.ev('return location.pathname;');
    ok(Math.abs(start - ziel.getTime()) < 20000 && route !== '/expenses', `CLOCK die Seite steht auf ${new Date(start).toString().slice(0, 24)} (Route ${route})`);
    await sleep(3000); await spuelen(primary);
    ok(S(monate(TID)) === S(['2026-09']), `BOOT der Start am 30.09. legt nichts an (${S(monate(TID))})`);
    // Warten bis nach Mitternacht (Seite) + höchstens ein Takt (60 s) + Puffer.
    let oktober = 0, seitenZeitBeiFund = 0;
    const ende = Date.now() + 150000;
    while (Date.now() < ende) {
      await sleep(3000);
      await spuelen(primary);
      oktober = monate(TID).filter((m) => m === '2026-10').length;
      if (oktober > 0) { seitenZeitBeiFund = await seitenZeit(); break; }
    }
    const nachMitternacht = seitenZeitBeiFund ? (seitenZeitBeiFund - new Date(2026, 9, 1, 0, 0, 0).getTime()) / 1000 : NaN;
    MESS.schedulerSekNachMitternacht = nachMitternacht;
    const routeDanach = await primary.ev('return location.pathname;');
    RV.schedulerOct = oktober === 1 && nachMitternacht >= 0 && nachMitternacht <= 75 && routeDanach === route;
    ok(RV.schedulerOct, `SCHEDULER der Oktober entstand ohne Klick/Neuladen/Ausgabenliste, ${nachMitternacht.toFixed(1)} s nach Mitternacht der Seite (≤ 1 Takt + Abfrage), genau einmal (${S(monate(TID))}; Route ${routeDanach})`);
    await sleep(65000); await spuelen(primary);
    RV.schedulerOnce = S(monate(TID)) === S(['2026-09', '2026-10']);
    ok(RV.schedulerOnce, `SCHEDULER ein weiterer Takt legt nichts nach (${S(monate(TID))})`);
    await primary.send('Page.removeScriptToEvaluateOnNewDocument', { identifier: skriptId }).catch(() => null);

    // Reguläres Beenden direkt nach einem frischen Eintrag — dann Neustart mit Uhr 02.12.: Nachholen ohne Duplikat.
    const probe = String(Date.now());
    await primary.ev(`localStorage.setItem('r7b_review_probe', ${S(probe)}); return 1;`);
    const pid = eigenerPrimary();
    primary.close(); primary = null;
    const zu = regulaerSchliessen(pid);
    const weg = await waitPidGone(pid, 90000);
    ok(zu && weg, `CLOSE der Primary schließt regulär (Fenster schließen) direkt nach dem Eintrag (${zu}/${weg})`);
    primary = await attach(APP_CDP, APP, appEnv());
    await waitInvoke(primary);
    const angemeldet = await warteBis(primary, `!!document.querySelector(${S(SHELL)})`, 90000);
    const gefunden = await primary.ev("return localStorage.getItem('r7b_review_probe');");
    RV.closeProbe = angemeldet && gefunden === probe;
    ok(RV.closeProbe, `CLOSE nach regulärem Beenden ohne Wartezeit: angemeldet und der frische Eintrag ist da (${gefunden === probe})`);
    await primary.ev(DIALOGE_PRIMARY);
    await uhrStellen(new Date(2026, 11, 2, 9, 0, 0));
    let nachgeholt = [];
    for (let i = 0; i < 20; i++) {
      await sleep(2000); await spuelen(primary);
      nachgeholt = monate(TID);
      if (nachgeholt.includes('2026-12')) break;
    }
    await sleep(3000); await spuelen(primary);
    nachgeholt = monate(TID);
    RV.restartCatchUp = S(nachgeholt) === S(['2026-09', '2026-10', '2026-11', '2026-12']);
    ok(RV.restartCatchUp, `RESTART nach dem Neustart (Uhr 02.12.) November und Dezember nachgeholt, jeder Monat genau einmal (${S(nachgeholt)})`);
    ok(zahl('SELECT COUNT(*) AS n FROM (SELECT recurring_template_id, substr(expense_date,1,7) m, COUNT(*) c FROM expenses WHERE recurring_template_id IS NOT NULL GROUP BY 1, 2 HAVING c > 1)') === 0,
      'RESTART kein Monat doppelt (über alle Daueraufträge)');
  }

  // ══════════════════════════════════════════════════════════════════════
  // PP-12 — Messung am Befehlsweg (ohne Oberfläche), größte Datei, wachsende Datenbank
  // ══════════════════════════════════════════════════════════════════════
  {
    const token = MESS.token;
    const DOC_MAX = 25_116_672;
    // Ein echtes PNG mit Text (von der Seite des Primary gezeichnet), auf die größte zulässige Länge aufgefüllt.
    const png = Buffer.from(String(await primary.ev(`const c=document.createElement('canvas'); c.width=720; c.height=180; const g=c.getContext('2d');
      g.fillStyle='#ffffff'; g.fillRect(0,0,720,180); g.fillStyle='#000000'; g.font='bold 64px Arial'; g.fillText('LATAIF R7B 4711', 24, 112);
    ...
    const frist = 20 + Math.floor((Math.min(url.length, 33_554_432) * 10 * 1000) / 10_000_000) / 1000;
    MESS.uploadFrist = frist;
    MESS.upload = [];
    let letzte = '';
    for (let i = 1; i <= 3; i++) {
      const r = await befehl('documents.upload', { fileName: `r7b-review-max-${i}.png`, content: url, docClass: 'other', linkedEntityType: null, linkedEntityId: null }, token);
      MESS.upload.push({ ms: Math.round(r.ms), status: r.status, ok: r.j?.ok === true, err: r.j?.error || null, db: (() => { try { return statSync(BIZ_DB).size; } catch { return 0; } })() });
      if (r.j?.ok) letzte = r.j.value.documentId;
    }
    RV.uploads = MESS.upload.every((u) => u.status === 200 && u.ok);
    ok(RV.uploads, `UPLOAD dreimal 25 116 672 B über /api/command: ${MESS.upload.map((u) => `${(u.ms / 1000).toFixed(2)} s`).join(' / ')} (Frist ${frist.toFixed(2)} s; ${S(MESS.upload)})`);
    const c = await befehl('documents.content.get', { documentId: letzte }, token);
    MESS.content = { ms: Math.round(c.ms), status: c.status, len: String(c.j?.value?.data?.content || c.j?.value?.content || '').length };
    RV.content = c.status === 200 && c.j?.ok === true;
    ok(RV.content, `CONTENT Inhalt der größten Datei zurück in ${(c.ms / 1000).toFixed(2)} s (Frist 33,42 s; ${S({ status: c.status, len: MESS.content.len })})`);
    ...
    const up = await befehl('documents.upload', { fileName: 'r7b-review-ocr.jpg', content: 'data:image/jpeg;base64,' + b64, docClass: 'other', linkedEntityType: null, linkedEntityId: null }, token);
    const o = await befehl('documents.set_ocr', { documentId: up.j?.value?.documentId, expectedRevision: up.j?.value?.revision }, token);
    MESS.ocr = { ms: Math.round(o.ms), status: o.status, text: String(o.j?.value?.text || '').replace(/\s+/g, ' ').slice(0, 40) };
    RV.ocr = o.status === 200 && o.j?.ok === true && /4711|R7B|LATAIF/i.test(MESS.ocr.text);
    ok(RV.ocr, `OCR 24 MP in ${(o.ms / 1000).toFixed(2)} s (Frist 90 s; ${S(MESS.ocr)})`);
  }
...
const ZEILE = `post-parity r7b review runtime: regular close keeps fresh storage, scheduler in the real app (month boundary, once, restart catch-up), bridge timings of the largest document and capped OCR (${Math.floor(dauer / 60)}m ${dauer % 60}s): ${PASS} passed, ${FAIL} failed`;
if (FAIL > 0) {
  console.log(`\nFAIL — ${ZEILE}`);
  for (const f of fails) console.log('  - ' + f);
  process.exit(1);
}
if (RV_STATE.closeLogin && RV_STATE.closeProbe) console.log('POST_PARITY_R7B_REVIEW_REGULAR_CLOSE_KEEPS_STORAGE_PROVED');
if (RV_STATE.schedulerOct && RV_STATE.schedulerOnce && RV_STATE.restartCatchUp) console.log('POST_PARITY_R7B_REVIEW_PP4_RUNTIME_PROVED');
if (RV_STATE.uploads && RV_STATE.content && RV_STATE.ocr) console.log('POST_PARITY_R7B_REVIEW_PP12_BRIDGE_TIMINGS_MEASURED');
console.log(`\nPASS — ${ZEILE}`);
