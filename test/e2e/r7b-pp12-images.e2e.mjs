// ════════════════════════════════════════════════════════════════════════════
// POST-PARITY R7B / PP-12 — Bildwege (gezielter Zwei-Rechner-Lauf).
// Run: node test/e2e/r7b-pp12-images.e2e.mjs
//
//   FERN     PC2-Transport (Ablage + /api/command, dasselbe Token-Protokoll wie PC2). PC2 rechnet Belegbilder VOR dem
//            Ablegen durch denselben Normalisierer (dieselbe Anwendung); der Primary prüft beim Abholen und übernimmt sie
//            Byte für Byte. Reparatur mit zwei Fotos, ein nicht normalisiertes Foto (rechnet der Primary), Wiederholung
//            derselben Kennung, Ersetzen (eins behalten, eins neu), Ausweisfoto, Altgold anlegen + ändern (das neu
//            abgelegte, schon gespeicherte Foto bleibt), Einkauf „New Item", Artikel über den Medienspeicher (Haupt + Vorschau).
//   PRIMARY  Die Maske der Reparatur am Primary: ein großes Kamerafoto über die echte Dateiauswahl (Aufnahmeprofil →
//            Normalisierer vor der Klammer); die gespeicherten Fotos bleiben unverändert.
//   ANZEIGE  Primary und PC2 (echte Oberfläche, angemeldet als B) zeigen die gespeicherten Fotos.
//   NEUSTART Reguläres Beenden, Neustart: jedes Foto Byte für Byte da, die Medien-Dateien auch.
//
// Die Frist bei großer Datenbank prüft ein eigener Lauf: test/e2e/r7b-pp12-large-db.e2e.mjs.
//
...
const OWNER_PW = 'pp12-images-owner-' + Math.random().toString(36).slice(2);
...
const RUN = join(os.tmpdir(), 'lataif-pp12-images', 'run-' + Date.now());
...
try {
  ...
  if (await exists(primary, 'input[placeholder="e.g. Al-Khalifa Luxury"]')) {
    await setVal(primary, 'input[placeholder="e.g. Al-Khalifa Luxury"]', 'PP12 Images Co');
  ...
  // ══════════════════════════════════════════════════════════════════════
  const PP = {
    pc2Repair: false, retry: false, primaryEdit: false, replace: false, supplier: false, scrapKeep: false, purchase: false,
    pipeline: false, displayPrimary: false, displayPc2: false, restart: false,
  };
  const MESS = {};
  RV_STATE = PP; MESS_STATE = MESS;
  ...
  /** Was in der Zeile steht: JPEG?, Bytes, Maße (aus dem SOF-Kopf), SHA-256, EXIF? */
  ...
  /** Ein großes, fotoähnliches JPEG (3000×2000, Qualität 0,92), gezeichnet von der Seite des Primary. */
  const foto = (seed, w = 3000, h = 2000) => primary.ev(`const c=document.createElement('canvas'); c.width=${w}; c.height=${h}; const g=c.getContext('2d');
    ...
    g.fillStyle='#ffffff'; g.font='bold 160px Arial'; g.fillText('LATAIF PP12 ${seed}', 120, 400);
    return c.toDataURL('image/jpeg', 0.92).split(',')[1];`);
  /** Was die Auswahl des Desktops daraus macht (`capture-profile.ts`: ≤ 800 px, JPEG 0,7) — so reist ein Foto von PC2. */
  const aufnahme = (seed, w = 800, h = 533, q = 0.7) => primary.ev(`const c=document.createElement('canvas'); c.width=${w}; c.height=${h}; const g=c.getContext('2d');
    ...
    const d=g.getImageData(0,0,${w},${h}); for(let i=0;i<d.data.length;i+=4){ const n=(r()-0.5)*40; d.data[i]+=n; d.data[i+1]+=n; d.data[i+2]+=n; } g.putImageData(d,0,0);
    g.fillStyle='#ffffff'; g.font='bold 48px Arial'; g.fillText('LATAIF PP12 ${seed}', 30, 90);
    return c.toDataURL('image/jpeg', ${q}).split(',')[1];`);
  /** PC2 rechnet ein Belegbild VOR dem Ablegen durch den Normalisierer — dieselbe Anwendung, derselbe Befehl. */
  ...
  const JPEGS = "[...document.querySelectorAll('img')].filter(i=>/^data:image\\/jpeg/.test(i.src)).length";
  const BELEGBILDER = (n) => `[...document.images].filter(i => i.complete && i.naturalWidth > 0 && /^data:image\\/jpeg/.test(i.src)).length >= ${n}`;
  ...
  ok(login.status === 200 && !!TOKEN, `SETUP Anmeldung am Primary-Server (dasselbe Protokoll wie PC2) (${login.status})`);
  const ablegen = async (b64) => {
  ...
  // f = Kamera-Originale (für die echte Dateiauswahl am Primary); a = die Aufnahmefassung, die PC2 hat;
  // n = was PC2 daraus vor dem Ablegen macht (derselbe Normalisierer).
  const f = [], a = [], n = [];
  for (const seed of [11, 23, 37, 51, 67]) f.push(String(await foto(seed)));
  for (const seed of [11, 23, 37, 51, 67]) a.push(String(await aufnahme(seed)));
  for (const b of a) n.push(String(await normalisiert(b)));
  MESS.eingangBytes = f.map((b) => Buffer.from(b, 'base64').length);
  MESS.aufnahmeBytes = a.map((b) => Buffer.from(b, 'base64').length);
  MESS.pc2NormalisiertBytes = n.map((b) => Buffer.from(b, 'base64').length);
  ok(!!KUNDE && MESS.eingangBytes.every((x) => x > 1_000_000) && n.every((b) => passt(jpegInfo('data:image/jpeg;base64,' + b))),
    `SETUP Originale 3000×2000 (${MESS.eingangBytes.join(' / ')} B), Aufnahme (${MESS.aufnahmeBytes.join(' / ')} B), auf PC2 normalisiert (${MESS.pc2NormalisiertBytes.join(' / ')} B), Kunde ${KUNDE}`);

  // ══════════════════════════════════════════════════════════════════════
  // FERN — Reparatur: zwei große Fotos, Wiederholung derselben Kennung
  // ══════════════════════════════════════════════════════════════════════
  let REP = '', b1 = [];
  {
    const s1 = await ablegen(n[0]), s2 = await ablegen(n[1]);
    const cid = crypto.randomUUID();
    const body = { customerId: KUNDE, issueDescription: 'PP12 Foto-Test', photos: [{ stagingId: s1 }, { stagingId: s2 }] };
    const rc = await befehl('repairs.create', body, TOKEN, cid);
    REP = String(rc.j?.value?.repairId || '');
    await spuelen(primary);
    b1 = bilder(REP).map(jpegInfo);
    MESS.repairCreateMs = Math.round(rc.ms);
    PP.pc2Repair = rc.status === 200 && b1.length === 2 && b1.every(passt) && b1[0].sha === bytesSha(n[0]) && b1[1].sha === bytesSha(n[1]);
    ok(PP.pc2Repair, `FERN Reparatur, zwei auf PC2 normalisierte Fotos: gespeichert ≤ 100 000 B, JPEG, ≤ 1600 px, ohne EXIF — der Primary prüft und übernimmt sie Byte für Byte (${rc.status} ${rc.j?.error || ''}; ${b1.map(kurz).join(' / ')}; ${MESS.repairCreateMs} ms)`);
    // Ein Rechner, der NICHT vorher normalisiert (alter Client): der Primary rechnet beim Abholen selbst.
    const roh = String(await aufnahme(89, 1000, 667, 0.95));
    const rohInfo = jpegInfo('data:image/jpeg;base64,' + roh);
    const rf = await befehl('repairs.create', { customerId: KUNDE, issueDescription: 'PP12 Foto-Test roh', photos: [{ stagingId: await ablegen(roh) }] }, TOKEN);
    await spuelen(primary);
    const rid = String(rf.j?.value?.repairId || '');
    const br = bilder(rid).map(jpegInfo);
    MESS.primaryFallback = { status: rf.status, ms: Math.round(rf.ms), roh: kurz(rohInfo), gespeichert: br.map(kurz) };
    ok(rf.status === 200 && rohInfo.bytes > 100000 && br.length === 1 && passt(br[0]) && br[0].sha !== rohInfo.sha,
      `FERN ein nicht normalisiertes Foto (${kurz(rohInfo)}) rechnet der Primary beim Abholen selbst → ${br.map(kurz)} (${rf.status} ${rf.j?.error || ''}; ${Math.round(rf.ms)} ms)`);
    const again = await befehl('repairs.create', body, TOKEN, cid);
    await spuelen(primary);
    const anzahl = Number(dbQ(BIZ_DB, "SELECT COUNT(*) AS n FROM repairs WHERE issue_description = 'PP12 Foto-Test'")[0]?.n);
    const b1b = bilder(REP).map(jpegInfo);
    PP.retry = again.status === 200 && String(again.j?.value?.repairId || '') === REP && anzahl === 1 && S(b1b.map((x) => x.sha)) === S(b1.map((x) => x.sha));
    ok(PP.retry, `FERN dieselbe Kennung erneut: dieselbe Reparatur, genau eine, die Fotos nicht neu gerechnet (${again.status}; ${anzahl})`);
  }

  // ══════════════════════════════════════════════════════════════════════
  // PRIMARY — die Maske: ein großes Foto über die echte Dateiauswahl
  // ══════════════════════════════════════════════════════════════════════
  const F4 = join(RUN, 'pp12-foto-4.jpg');
  writeFileSync(F4, Buffer.from(f[3], 'base64'));
  {
    await gehFrisch(primary, `/repairs/${REP}`);
    const r = [];
    r.push((await warteBis(primary, knopfDa('Edit'), 30000)) ? 'OK' : 'KEIN-EDIT');
    if (r[0] === 'OK') r.push(await clickText(primary, 'Edit'));
    r.push((await warteBis(primary, "document.querySelector('[data-repair-save]')", 15000)) ? 'OK' : 'KEINE-MASKE');
    r.push(await fotoWaehlen(primary, F4));
    r.push((await warteBis(primary, `${JPEGS} >= 3`, 20000)) ? 'OK' : 'KEIN-VORSCHAUBILD');
    if (alleOk(r) === 'OK') {
      await click(primary, '[data-repair-save]');
      r.push((await warteBis(primary, "!document.querySelector('[data-repair-save]')", 60000)) ? 'OK' : 'NICHT-GESPEICHERT:' + await fehlerText(primary));
    }
    await spuelen(primary);
    const b2 = bilder(REP).map(jpegInfo);
    MESS.primaryEdit = b2.map(kurz);
    PP.primaryEdit = alleOk(r) === 'OK' && b2.length === 3 && b2[0].sha === b1[0]?.sha && b2[1].sha === b1[1]?.sha && passt(b2[2]);
    ok(PP.primaryEdit, `PRIMARY Maske: ein großes Foto (${MESS.eingangBytes[3]} B) über die echte Dateiauswahl → gespeichert ≤ 100 000 B; die zwei gespeicherten Byte für Byte unverändert (${alleOk(r)}; ${b2.map(kurz).join(' / ')})`);
  }

  // ══════════════════════════════════════════════════════════════════════
  // FERN — Ersetzen: eins behalten, die anderen weg, eins neu
  // ══════════════════════════════════════════════════════════════════════
  {
    const rev = Number(dbQ(BIZ_DB, 'SELECT revision FROM repairs WHERE id = ?', [REP])[0]?.revision ?? 0);
    const vorher = bilder(REP).map(jpegInfo);
    const s5 = await ablegen(n[4]);
    const ru = await befehl('repairs.update', { id: REP, expectedRevision: rev, photos: [{ keep: 0 }, { stagingId: s5 }] }, TOKEN);
    await spuelen(primary);
    const b3 = bilder(REP).map(jpegInfo);
    PP.replace = ru.status === 200 && b3.length === 2 && b3[0].sha === vorher[0]?.sha && passt(b3[1]) && !vorher.some((x) => x.sha === b3[1].sha);
    ok(PP.replace, `FERN Ersetzen: das erste bleibt Byte für Byte, zwei gehen, ein neues ≤ 100 000 B (${ru.status} ${ru.j?.error || ''}; ${b3.map(kurz).join(' / ')})`);
  }

  // ══════════════════════════════════════════════════════════════════════
  // FERN — Ausweisfoto, Altgold (behalten per SHA-256), Einkauf „New Item"
  // ══════════════════════════════════════════════════════════════════════
  let LIEF_NEU = '';
  {
    const s = await ablegen(n[2]);
    const r = await befehl('suppliers.create', { name: 'PP12 Lieferant Ausweis', cprImageStagingId: s }, TOKEN);
    LIEF_NEU = String(r.j?.value?.supplierId || '');
    await spuelen(primary);
    const x = jpegInfo(dbQ(BIZ_DB, 'SELECT cpr_image FROM suppliers WHERE id = ?', [LIEF_NEU])[0]?.cpr_image || '');
    PP.supplier = r.status === 200 && passt(x) && x.sha === bytesSha(n[2]);
    ok(PP.supplier, `FERN Ausweisfoto eines neuen Lieferanten: gespeichert ≤ 100 000 B (${r.status} ${r.j?.error || ''}; ${kurz(x)})`);
  }
  let TRADE = '';
  {
    const felder = (purchase, sale) => ({
      sellerName: 'PP12 Verkaeufer', buyerName: 'PP12 Kaeufer', tradeDate: '2026-09-15',
      lines: [{ weightGrams: 5, karat: '21K', purchasePrice: 100, salePrice: 120, ...(purchase.length ? { purchaseStagingIds: purchase } : {}), ...(sale.length ? { saleStagingIds: sale } : {}) }],
      paymentsOut: [{ method: 'cash', amount: 100 }], paymentsIn: [{ method: 'cash', amount: 120 }],
    });
    const rc = await befehl('scrap_trades.create', felder([await ablegen(n[1])], []), TOKEN);
    await spuelen(primary);
    const tr = dbQ(BIZ_DB, "SELECT id, version FROM scrap_trades WHERE seller_name = 'PP12 Verkaeufer' ORDER BY created_at DESC LIMIT 1")[0] || {};
    TRADE = String(tr.id || '');
    const zeile = () => dbQ(BIZ_DB, 'SELECT images_purchase, images_sale FROM scrap_trade_lines WHERE scrap_trade_id = ? ORDER BY position', [TRADE])[0] || {};
    const vorher = JSON.parse(String(zeile().images_purchase || '[]'));
    const v1 = vorher.map(jpegInfo);
    // PC2 legt beim Ändern auch das schon gespeicherte Foto neu ab — genau dessen Bytes.
    const sAlt = await ablegen(String(vorher[0] || '').split(',')[1] || '');
    const sNeu = await ablegen(n[4]);
    const ru = await befehl('scrap_trades.update', { tradeId: TRADE, expectedVersion: Number(tr.version), ...felder([sAlt], [sNeu]) }, TOKEN);
    await spuelen(primary);
    const nach = zeile();
    const p2 = JSON.parse(String(nach.images_purchase || '[]')).map(jpegInfo);
    const s2 = JSON.parse(String(nach.images_sale || '[]')).map(jpegInfo);
    PP.scrapKeep = rc.status === 200 && ru.status === 200 && v1.length === 1 && passt(v1[0]) && p2.length === 1 && p2[0].sha === v1[0].sha && s2.length === 1 && passt(s2[0]);
    ok(PP.scrapKeep, `FERN Altgold: angelegt ≤ 100 000 B; beim Ändern bleibt das neu abgelegte, schon gespeicherte Foto Byte für Byte, das neue ≤ 100 000 B (${rc.status}/${ru.status} ${ru.j?.error || ''}; ${v1.map(kurz)} → ${p2.map(kurz)} + ${s2.map(kurz)})`);
  }
  let P_EINKAUF = '';
  {
    const s = await ablegen(n[2]);
    const lief = String(dbQ(BIZ_DB, 'SELECT id FROM suppliers WHERE COALESCE(active, 1) = 1 AND id != ? ORDER BY id LIMIT 1', [LIEF_NEU])[0]?.id || '');
    const body = {
      supplierId: lief, purchaseDate: '2026-09-15', taxScheme: 'ZERO', paymentAmount: 0, paymentMethod: 'cash', notes: '', staffId: '',
      lines: [{ mode: 'new', brand: 'PP12', name: 'Einkauf mit Foto', sku: '', categoryId: 'pp14-cat', quantity: 1, unitPrice: 200,
        newProduct: { categoryId: 'pp14-cat', brand: 'PP12', name: 'Einkauf mit Foto', stagingIds: [s] } }],
    };
  ...
  // ══════════════════════════════════════════════════════════════════════
  // FERN — Artikel mit Foto über den Medienspeicher: Hauptbild + Vorschau
  // ══════════════════════════════════════════════════════════════════════
  let P_MEDIA = '';
  ...
  {
    const s = await ablegen(a[3]);
    const r = await befehl('products.create', { categoryId: 'pp14-cat', brand: 'PP12', name: 'Artikel Medienspeicher', stagingIds: [s] }, TOKEN);
    await spuelen(primary);
    P_MEDIA = String(dbQ(BIZ_DB, "SELECT id FROM products WHERE name = 'Artikel Medienspeicher' ORDER BY created_at DESC LIMIT 1")[0]?.id || '');
    const main = dbQ(BIZ_DB, MEDIA_MAIN, [P_MEDIA])[0] || {};
    const thumb = dbQ(BIZ_DB, MEDIA_THUMB, [P_MEDIA])[0] || {};
    const legacy = dbQ(BIZ_DB, 'SELECT images FROM products WHERE id = ?', [P_MEDIA])[0]?.images;
    MESS.pipeline = { ms: Math.round(r.ms), main: [main.bytes, main.w, main.h, datei(main.key)], thumb: [thumb.bytes, thumb.w, thumb.h, datei(thumb.key)] };
    PP.pipeline = r.status === 200 && Number(main.bytes) > 0 && Number(main.bytes) <= 100000 && datei(main.key) === Number(main.bytes)
      && Number(thumb.bytes) > 0 && Number(thumb.bytes) <= 20000 && datei(thumb.key) === Number(thumb.bytes)
      && Math.max(Number(main.w), Number(main.h)) <= 1600 && Math.max(Number(thumb.w), Number(thumb.h)) <= 256 && legacy === '[]';
    ok(PP.pipeline, `FERN Artikel mit Foto: Hauptbild ≤ 100 000 B und Vorschau ≤ 20 000 B als Dateien im Medienspeicher, products.images leer (${r.status}; ${S(MESS.pipeline)})`);
  }
  ...
  {
    await gehFrisch(primary, `/repairs/${REP}`);
    const rep = await warteBis(primary, BELEGBILDER(2), 30000);
    await gehFrisch(primary, `/collection/${P_EINKAUF}`);
    const pe = await warteBis(primary, EIN_BILD, 30000);
    await gehFrisch(primary, `/collection/${P_MEDIA}`);
    const pm = await warteBis(primary, EIN_BILD, 30000);
    PP.displayPrimary = rep && pe && pm;
    ok(PP.displayPrimary, `ANZEIGE Primary: Reparaturfotos, Einkaufsartikel, Artikel aus dem Medienspeicher (${[rep, pe, pm]})`);
  }
  ...
  // ══════════════════════════════════════════════════════════════════════
  {
    ...
    await geh(client, `/repairs/${REP}`); await sleep(2500);
    const rep = await warteBis(client, BELEGBILDER(2), 40000);
    await geh(client, `/collection/${P_EINKAUF}`); await sleep(2500);
    const pe = await warteBis(client, EIN_BILD, 40000);
    await geh(client, `/collection/${P_MEDIA}`); await sleep(2500);
    const pm = await warteBis(client, EIN_BILD, 40000);
    PP.displayPc2 = rep && pe && pm;
    ok(PP.displayPc2, `ANZEIGE PC2: Reparaturfotos, Einkaufsartikel, Artikel aus dem Medienspeicher (${[rep, pe, pm]})`);
    // PC2 geht wieder: seine Abfragen sollen die Zeitmessung am Primary nicht mitbestimmen (nur der eigene Test-Pfad).
    client.close(); client = null;
    killTestImage('lataif-e2e-client.exe'); await waitTestImageGone('lataif-e2e-client.exe');
  }

  // ══════════════════════════════════════════════════════════════════════
  // NEUSTART — reguläres Beenden, dann jedes Foto Byte für Byte da
  // ══════════════════════════════════════════════════════════════════════
  {
    const stand = () => S([
    ...
    const weg = await waitPidGone(pid, 120000);
    primary = await attach(APP_CDP, APP, appEnv());
    await waitInvoke(primary);
    const angemeldet = await warteBis(primary, `!!document.querySelector(${S(SHELL)})`, 120000);
    await primary.ev(DIALOGE_PRIMARY);
    const nach = stand();
    await gehFrisch(primary, `/repairs/${REP}`);
    const sicht = await warteBis(primary, BELEGBILDER(2), 30000);
    await gehFrisch(primary, `/collection/${P_MEDIA}`);
    const sicht2 = await warteBis(primary, EIN_BILD, 30000);
    PP.restart = zu && weg && angemeldet && vor === nach && sicht && sicht2;
    ok(PP.restart, `NEUSTART regulär beendet und neu gestartet: jedes Foto Byte für Byte da, Medien-Dateien unverändert, Anzeige wieder da (${zu}/${weg}/${angemeldet}/${vor === nach}/${sicht}/${sicht2})`);
  }
...
console.log('\n  PP-12 Bildwege                    Ergebnis');
for (const [k, v] of Object.entries(RV_STATE)) console.log(`  ${k.padEnd(34)}${v === true ? 'ja' : 'NEIN'}`);
console.log(`  Messung: ${JSON.stringify({ ...MESS_STATE, token: undefined })}`);
const dauer = Math.round((Date.now() - T0) / 1000);
const ZEILE = `post-parity r7b pp-12 images: record photos through the one normalizer (capturing machine, primary checks on pickup), keep/replace/retry, pipeline main+thumbnail, display on primary and pc2, regular restart (${Math.floor(dauer / 60)}m ${dauer % 60}s): ${PASS} passed, ${FAIL} failed`;
if (FAIL > 0) {
  console.log(`\nFAIL — ${ZEILE}`);
...
const R = RV_STATE;
if (R.pc2Repair && R.retry && R.primaryEdit && R.replace && R.supplier && R.scrapKeep && R.purchase) console.log('POST_PARITY_PP12_RECORD_IMAGES_RUNTIME_PROVED');
if (R.pipeline) console.log('POST_PARITY_PP12_PIPELINE_MAIN_AND_THUMBNAIL_PROVED');
if (R.displayPrimary && R.displayPc2 && R.restart) console.log('POST_PARITY_PP12_DISPLAY_AND_RESTART_PROVED');
console.log(`\nPASS — ${ZEILE}`);
