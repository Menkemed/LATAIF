...
// ── §1 Der Umfang: genau die drei Transferzeilen der Klasse B, keine neue Buchung ──
{
  ...
  ok(S(nochB) === S(vorher.filter((op) => !op.startsWith('transfers.')).sort()),
    `SCOPE die uebrigen acht Klasse-B-Zeilen sind unberuehrt (${nochB.join(', ')})`);
  ...
  ok(S(stand) === S([30, 0, 8, 2]), `SCOPE die Matrix steht bei 30/0/8/2 (${stand.join('/')})`);
}
