...
// ── 1) Die Zulassungsliste: genau EINE Mutation ───────────────────────────
{
  ...
  // CENTRAL-UI-PARITY — die 30 Store-Auskuenfte gehoeren zum ausgelieferten Zustand: der
  ...
  ok(known.length === 89, `ALLOWLIST produktiv neunundachtzig Namen (${known.length})`);
  // CENTRAL-UI-PARITY — 18 Auskuenfte aus C2 plus 30 typisierte Auskuenfte fuer die gemeinsame Oberflaeche.
  ok(reads.length === 48 && known.includes('bridge.probe') && known.includes('invoices.create'),
    `ALLOWLIST eine Probe, achtundvierzig Lesevorgaenge, vierzig Mutationen (${reads.length})`);
  ...
  ok((list.match(/OP_[A-Z_]+/g) || []).length === 89, 'ALLOWLIST …und seine Liste ist genau neunundachtzig Namen lang');
