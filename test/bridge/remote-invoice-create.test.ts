...
// ── 1) Die Zulassungsliste: genau EINE Mutation ───────────────────────────
{
  ...
  ok(known.length === 174, `ALLOWLIST produktiv einhundertvierundsiebzig Namen (R6F) (${known.length})`);
  ...
  ok(reads.length === 71 && known.includes('bridge.probe') && known.includes('invoices.create'),
    `ALLOWLIST eine Probe, einundsiebzig Lesevorgaenge, hundertzwei Mutationen (${reads.length})`);
  ...
  ok((list.match(/OP_[A-Z_]+/g) || []).length === 174, 'ALLOWLIST …und seine Liste ist genau einhundertvierundsiebzig Namen lang');
