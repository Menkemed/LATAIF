...
// ── 10) Der Umfang ist unveraendert ───────────────────────────────────────
{
  ...
  ok(known.length === 27 && reads.length === 14 && ALLOWED_MUTATIONS.length === 12,
    `SCOPE 1 Probe + 14 Reads + 12 Mutationen = 27 (${known.length}/${reads.length}/${ALLOWED_MUTATIONS.length})`);
