...
// ── 1) Der Umfang bleibt eingefroren ─────────────────────────────────────
{
  ...
  ok(known.length === 106 && reads.length === 65 && parityReads.length === 47,
    `SCOPE 1 Probe + 18 Auskuenfte + 47 typisierte Auskuenfte + 40 Buchungen = 106 (${known.length}/${reads.length}/${parityReads.length})`);
  const rust = src('src-tauri/src/bridge.rs');
  const rl = rust.slice(rust.indexOf('pub const REMOTE_OPS'), rust.indexOf('];', rust.indexOf('pub const REMOTE_OPS')));
  ok((rl.match(/OP_[A-Z_]+/g) ?? []).length === 106, 'SCOPE Rust kennt dieselben 106');
