...
// ── 1) Umfang: was freigegeben ist — und was ausdrücklich nicht ───────────
{
  ...
  ok(list.length === 103, `SCOPE genau 103 Mutationen sind freigegeben (R7A) (${list.length})`);
  ...
  for (const op of [
    // R6F — `purchases.cancel` ist seither freigegeben (Einkaufsstorno als EINE Hausfolge).
    'purchases.update', 'purchases.add_payment', 'purchases.create_return',
