...
// ── 9) Die Zulassungsliste: genau fünf Namen ──────────────────────────────
{
  ...
  ok(known.length === 177 && reads.length === 72 && parityReads.length === 53 && known.includes('bridge.probe'),
  ...
  ok((list.match(/OP_[A-Z_]+/g) || []).length === 177, 'ALLOWLIST Rust kennt dieselben einhundertsechsundsiebzig Namen (PRE-G5)');
