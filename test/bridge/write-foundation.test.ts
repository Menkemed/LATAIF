...
// ── 12) Die Verdrahtung: kein alter Nummerngeber mehr, kein offenes Tor ────
{
  ...
  ok(ops.length === 175, `WIRED die Liste zaehlt einhundertfuenfundsiebzig Namen (R7A) (${ops.length})`);
  const wiredMut = ops.filter((o) => o !== 'bridge.probe' && !o.endsWith('.list') && !o.endsWith('.get'));
  ok(ops.filter((o) => o.endsWith('.list') || o.endsWith('.get')).length === 71 && ops.includes('bridge.probe')
    ...
    `WIRED eine Probe, sechsundsechzig Lesevorgaenge, VIERZIG Mutationen (${wiredMut.join(', ')})`);
  const engine = src('src/core/bridge/mutation-engine.ts');
