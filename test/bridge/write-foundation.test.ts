...
// ── 12) Die Verdrahtung: kein alter Nummerngeber mehr, kein offenes Tor ────
{
  ...
  // C3B hat das Tor NICHT aufgemacht, sondern EINEN Namen freigegeben; C3C hat zwei weitere
  // einzeln danebengelegt. Der Unterschied ist der ganze Punkt: ein Schalter haette in einem Zug
  // jede kuenftige Mutation mitgenommen — auch die, die nie jemand geprueft hat.
  ...
  ok(ops.length === 177, `WIRED die Liste zaehlt einhundertsechsundsiebzig Namen (PRE-G5) (${ops.length})`);
  const wiredMut = ops.filter((o) => o !== 'bridge.probe' && !o.endsWith('.list') && !o.endsWith('.get'));
  ok(ops.filter((o) => o.endsWith('.list') || o.endsWith('.get')).length === 72 && ops.includes('bridge.probe')
    ...
    `WIRED eine Probe, zweiundsiebzig Lesevorgaenge, VIERZIG Mutationen (${wiredMut.join(', ')})`);
  const engine = src('src/core/bridge/mutation-engine.ts');
...
// daran hält, verschachtelt sich sauber; wer selbst `BEGIN` ruft, sprengt die äußere Klammer.
{
  ...
  // R6D — Einkaufszahlung und Lieferantenguthaben setzen ihre Klammer seither in der gemeinsamen
  // Hausfolge (`core/payables/payables-house.ts`); der Store ruft sie nur noch.
  for (const f of outerToday) {
    const klammer = /beginLedgerTransaction/.test(src(`src/stores/${f}.ts`))
      || (['purchaseStore', 'supplierStore'].includes(f) && /payables-house/.test(src(`src/stores/${f}.ts`))
        && /beginLedgerTransaction/.test(src('src/core/payables/payables-house.ts')))
      // R6E — der Retourenstorno läuft seither als Hausfolge in der Klammer von `runOnPrimary`.
      || (f === 'salesReturnStore' && /return-cancel-house/.test(src(`src/stores/${f}.ts`))
        && /beginLedgerTransaction/.test(src('src/core/data/primary-action.ts')));
    ok(klammer, `TXAUDIT ${f} setzt heute schon eine aeussere Klammer`);
  }
