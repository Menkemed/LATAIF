...
// ── 3) Gelesen wird aus der Autoritaet, nicht von der Platte ──────────────
{
  ...
  ok(registered === 19, `AUTH neunzehn Leseoperationen (${registered})`);
...
// ── 6) Nichts Veraenderndes, auch nicht spaeter ───────────────────────────
{
  ...
  ok(registeredReads === 19, `READONLY plus neunzehn Lesevorgaenge im Quelltext (${registeredReads})`);
...
// Geschäftsdaten an.
{
  ...
  const ERLAUBT = {
    ...
    // R6F — ein Anschluss: die Hausfolge läuft über `createProductionOnPrimary` → `runOnPrimary` (Schreibspur).
    'src/stores/productionStore.ts:createRecord': 'Anschluss an createProductionOnPrimary (runOnPrimary)',
  };
...
// Es sind sechs Lesevorgänge plus die Probe.
{
  ...
  ok(resolved.length === 177, `ALLOWLIST einhundertsechsundsiebzig Namen (PRE-G5) insgesamt (${resolved.length})`);
  ...
  // CENTRAL-UI-PARITY — achtzehn Auskuenfte aus C2, dazu 48 typisierte Auskuenfte fuer die
  // gemeinsame Oberflaeche. Beides ist Lesen; getrennt gezaehlt, damit die Herkunft sichtbar bleibt.
  ok(reads.length === 72 && storeReads.length === 53,
    `ALLOWLIST 19 Auskuenfte + 53 typisierte Auskuenfte (${reads.length}/${storeReads.length})`);
  ...
  ok(resolved.join(',') === 'bridge.probe,products.list,products.get,customers.list,customers.get,invoices.list,invoices.get,invoices.create,customers.create,customers.update,products.create,products.update,invoices.update,invoices.record_payment,suppliers.list,categories.list,purchases.list,purchases.get,consignments.list,consignments.get,orders.list,orders.get,purchases.create,consignments.create,consignments.update,orders.create,orders.update,repairs.list,repairs.get,transfers.list,transfers.get,repairs.create,repairs.update,transfers.create,transfers.update,transfers.mark_returned,invoices.apply_credit,invoices.update_payment,invoices.delete_payment,orders.convert_to_invoice,consignments.record_payout,transfers.mark_sold,transfers.mark_settled,returns.create,returns.approve,returns.refund,returns.record_refund_payment,orders.update_status,orders.add_payment,orders.delete_payment,consignments.record_sale,consignments.mark_returned,repairs.update_status,repairs.create_invoice,repairs.add_line,repairs.update_line,repairs.cancel_line,transfers.convert_to_invoice,transfers.convert_many_to_invoice,invoices.cancel,store.products.get,store.customers.get,store.invoices.get,order_payments.get,session.context.get,store.suppliers.get,store.sales_returns.get,store.credit_notes.get,store.orders.get,store.consignments.get,store.purchases.get,store.repairs.get,store.agents.get,store.expenses.get,store.recurring_expenses.get,store.banking.get,store.payables.get,store.debts.get,store.gold.get,store.metals.get,store.scrap_trades.get,store.employees.get,store.partners.get,store.tasks.get,store.documents.get,store.offers.get,store.production.get,store.analytics.get,analytics.vat_export.get,documents.content.get,page.dashboard.get,page.invoice_list.get,page.order_list.get,page.customer_detail.get,page.order_detail.get,page.supplier_detail.get,page.product_detail.get,page.purchase_create.get,refs.numbers.get,metals.stock_by_karat.get,search.global.get,page.reconciliation.get,ledger.balances.get,finance.receivables.get,inventory.lot_aggregates.get,product.lots.get,product.lots.batch.get,expenses.credit_paid.get,suppliers.create,suppliers.update,agents.update,partners.create,partners.update,employees.create,employees.update,inventory.start,inventory.save,inventory.finish,inventory.record_check,inventory.session.get,inventory.checks.get,tax.record_payment,banking.transfer,partners.record_tx,debts.create,debts.update,debts.record_payment,expenses.create,expenses.update,expenses.record_payment,expenses.template_create,expenses.template_update,purchases.record_payment,purchases.apply_credit,suppliers.pay,suppliers.apply_credit,suppliers.refund_credit,gold.payables.settle,gold.customer_credits.settle,repairs.record_gold_usage,repairs.add_material,orders.add_cost,orders.remove_cost,metals.create,metals.update_status,metals.set_spot_price,scrap_trades.create,scrap_trades.update,scrap_trades.cancel,metals.spot_prices.get,debts.payments.get,suppliers.credits.get,offers.create,offers.update,offers.set_status,offers.convert_to_invoice,invoices.set_butterfly,returns.cancel,transfers.undo_convert,customers.log_message,purchases.return_to_supplier,purchases.cancel,purchases.dismiss_inbox,orders.cancel,orders.update_line_status,orders.mark_line_ordered,orders.update_line,consignments.return_after_sale,consignments.cancel_sale,production.create,tasks.create,tasks.update,documents.upload,documents.set_ocr,production.complete,products.duplicates.get,purchase_inbox.create',
    `ALLOWLIST in dieser Reihenfolge (${resolved.join(',')})`);
  // Die Artikelsuche ist ein PARAMETER von `products.list` und bekommt keine eigene Operation.
  // Seit R2D gibt es GENAU EINE Suchoperation, und sie ist etwas anderes: die uebergreifende
  // Suche ueber Artikel, Kunden und sieben Belegarten, die vorher als neun Abfragen in einer
  // Komponente lag. Eine zweite waere ein Rueckschritt.
  const suchOps = resolved.filter((o) => /search/.test(o));
  ok(suchOps.join(',') === 'search.global.get', `ALLOWLIST genau eine Suchoperation, und zwar die uebergreifende (${suchOps.join(',') || 'keine'})`);
