...
// Es sind sechs Lesevorgänge plus die Probe.
{
  ...
  ok(resolved.length === 106, `ALLOWLIST einhundertsechs Namen insgesamt (${resolved.length})`);
  ...
  // CENTRAL-UI-PARITY — achtzehn Auskuenfte aus C2, dazu 47 typisierte Auskuenfte fuer die
  // gemeinsame Oberflaeche. Beides ist Lesen; getrennt gezaehlt, damit die Herkunft sichtbar bleibt.
  ok(reads.length === 65 && storeReads.length === 47,
    `ALLOWLIST 18 Auskuenfte + 47 typisierte Auskuenfte (${reads.length}/${storeReads.length})`);
