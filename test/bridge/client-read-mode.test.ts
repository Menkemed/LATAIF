...
// Es sind sechs Lesevorgänge plus die Probe.
{
  ...
  ok(resolved.length === 72, `ALLOWLIST zweiundsiebzig Namen insgesamt (${resolved.length})`);
  ...
  // CENTRAL-UI-PARITY — achtzehn Auskuenfte aus C2, dazu 13 typisierte Auskuenfte fuer die
  // gemeinsame Oberflaeche. Beides ist Lesen; getrennt gezaehlt, damit die Herkunft sichtbar bleibt.
  ok(reads.length === 31 && storeReads.length === 13,
    `ALLOWLIST 18 Auskuenfte + 13 typisierte Auskuenfte (${reads.length}/${storeReads.length})`);
