...
// Es sind sechs Lesevorgänge plus die Probe.
{
  ...
  // CENTRAL-UI-PARITY: dazu 42 Store-Auskuenfte, mit denen PC2 DIESELBE Oberflaeche fuellt
  ok(resolved.length === 101, `ALLOWLIST einhundertein Namen insgesamt (${resolved.length})`);
  ...
  // CENTRAL-UI-PARITY — achtzehn Auskuenfte aus C2, dazu 42 typisierte Auskuenfte fuer die
  // gemeinsame Oberflaeche. Beides ist Lesen; getrennt gezaehlt, damit die Herkunft sichtbar bleibt.
  ok(reads.length === 60 && storeReads.length === 42,
    `ALLOWLIST 18 Auskuenfte + 42 typisierte Auskuenfte (${reads.length}/${storeReads.length})`);
