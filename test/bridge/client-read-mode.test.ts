...
// Es sind sechs Lesevorgänge plus die Probe.
{
  ...
  ok(resolved.length === 64, `ALLOWLIST vierundsechzig Namen insgesamt (${resolved.length})`);
  ok(probes.length === 1, `ALLOWLIST genau eine Probe (${probes.length})`);
  const storeReads = reads.filter((o) => ['store.products.get', 'store.customers.get', 'store.invoices.get', 'order_payments.get', 'session.context.get'].includes(o));
  // CENTRAL-UI-PARITY — achtzehn Auskuenfte aus C2, dazu 5 typisierte Auskuenfte fuer die
  // gemeinsame Oberflaeche. Beides ist Lesen; getrennt gezaehlt, damit die Herkunft sichtbar bleibt.
  ok(reads.length === 23 && storeReads.length === 5,
    `ALLOWLIST 18 Auskuenfte + 5 typisierte Auskuenfte (${reads.length}/${storeReads.length})`);
