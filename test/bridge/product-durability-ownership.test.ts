...
// ── 7) Die Verdrahtung im Produktcode ─────────────────────────────────────
{
  ...
  // R7A-Triage: die geladenen Module verdrahten weiter genau 60 Befehle; die Registry selbst ist seit
  // R5F.1 gewachsen (R6A–R6F 102, R7A 103 Buchungen). Der R5F.1-Kern steht unveraendert VORN.
  ok(known.length === 60 && registry.ALLOWED_MUTATIONS.length === 104 && registry.ALLOWED_MUTATIONS.join(',').startsWith('invoices.create,customers.create,customers.update,products.create,products.update,invoices.update,invoices.record_payment,purchases.create,consignments.create,consignments.update,orders.create,orders.update,repairs.create,repairs.update,transfers.create,transfers.update,transfers.mark_returned,invoices.apply_credit,invoices.update_payment,invoices.delete_payment,orders.convert_to_invoice,consignments.record_payout,transfers.mark_sold,transfers.mark_settled,returns.create,returns.approve,returns.refund,returns.record_refund_payment,orders.update_status,orders.add_payment,orders.delete_payment,consignments.record_sale,consignments.mark_returned,repairs.update_status,repairs.create_invoice,repairs.add_line,repairs.update_line,repairs.cancel_line,transfers.convert_to_invoice,transfers.convert_many_to_invoice,invoices.cancel,'),
    `WIRED 1 Probe + 19 Reads + 40 Mutationen (${known.length}: ${registry.ALLOWED_MUTATIONS.join(', ')})`);
