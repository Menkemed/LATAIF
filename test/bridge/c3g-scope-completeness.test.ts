...
// ── 3) Keine Klasse-C-Aktion ist registriert ─────────────────────────────
{
  ...
  ok(list.length === 104, `REGISTRY genau 104 Mutationen (${list.length}) — 24 aus C3G, 16 aus C3H, invoices.cancel, 11 aus R6C, 28 aus R6D, 8 aus R6E, 14 aus R6F, 1 aus R7A (production.complete)`);
