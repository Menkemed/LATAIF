...
async fn a_name_that_is_not_allow_listed_never_reaches_the_renderer() {
    ...
    assert_eq!(
        REMOTE_OPS.len(),
        160,
        "Probe, einundsiebzig Lesevorgaenge und achtundachtzig Buchungen (R6C: sieben Stammdaten, vier Inventur, zwei Inventur-Auskuenfte; R6D: achtundzwanzig Geld/Steuer/Gold/Metall, drei Auskuenfte; R6E: acht Angebot/Rechnung/Retoure/Transfer/Nachricht)"
    );
    for op in [
        OP_INVOICES_CREATE,
        OP_CUSTOMERS_CREATE,
    ...
    assert_eq!(
        ...
        "und NUR diese achtundachtzig veraendern etwas — kein Loeschen von aussen, kein Storno mit Geld
    ...
    for forbidden in [
        ...
        "consignments.delete",
        "repairs.delete",
        // R6E — `transfers.undo_convert` und `returns.cancel` sind seither ausdruecklich freigegeben:
        // die Umwandlung wird als Rechnungsstorno zurueckgenommen (die Rechnung bleibt als Beleg), der
        // Retourenstorno bleibt dem Eigentuemer vorbehalten. Loeschen bleibt Primary-only.
        "transfers.delete",
    ] {
        assert!(
...
fn the_route_takes_a_client_command_id_and_reports_the_outcome_class() {
    ...
    assert!(
        ...
        "genau diese achtundachtzig veraendernden Namen sind freigegeben — und keiner mehr (R6C: sieben Stammdaten, vier Inventur; R6D: achtundzwanzig Geld/Steuer/Gold/Metall; R6E: acht)"
    );
    assert!(
