...
async fn a_name_that_is_not_allow_listed_never_reaches_the_renderer() {
    ...
    assert_eq!(
        REMOTE_OPS.len(),
        152,
        "Probe, einundsiebzig Lesevorgaenge und achtzig Buchungen (R6C: sieben Stammdaten, vier Inventur, zwei Inventur-Auskuenfte; R6D: achtundzwanzig Geld/Steuer/Gold/Metall, drei Auskuenfte)"
    );
    for op in [
        OP_INVOICES_CREATE,
        OP_CUSTOMERS_CREATE,
    ...
    assert_eq!(
        ...
        "und NUR diese achtzig veraendern etwas — kein Loeschen von aussen, kein Storno mit Geld,
...
fn the_route_takes_a_client_command_id_and_reports_the_outcome_class() {
    ...
    assert!(
        ...
        "genau diese achtzig veraendernden Namen sind freigegeben — und keiner mehr (R6C: sieben Stammdaten, vier Inventur; R6D: achtundzwanzig Geld/Steuer/Gold/Metall)"
    );
    assert!(
