...
export const usePayablesStore = create<PayablesStore>((set) => ({
  ...
  loadPayables: () => {
    ...
    try {
      set({ ...loadPayablesFor(localReadContext()), loading: false });
    } catch (e) {
...
/**
 * CENTRAL-UI-PARITY R2B — die offenen Verbindlichkeiten einer Filiale, zustandsfrei.
 *
 * Diese Liste hatte bis hierher UEBERHAUPT KEINE Filialgrenze: sie zaehlte Einkaeufe,
 * Retouren, Kommissionen, Ausgaben und Darlehen aller Filialen zusammen. Solange nur der
 * Primary sie sah, fiel das im Ein-Filial-Betrieb nicht auf; als Fernauskunft waere es die
 * Preisgabe fremder Zahlen. Status, Betrag und Faelligkeit sind AUSWAHL — die Berechtigung
 * kommt allein aus dem Ausweis, und der steht jetzt in jeder der fuenf Abfragen.
 */
export function loadPayablesFor(ctx: BusinessReadContext): { payables: PayableRow[] } {
  const today = new Date();
  // Auch die Einstellung gehoert der Filiale des Anfragenden, nicht der des Primary.
  const graceRows = query(
