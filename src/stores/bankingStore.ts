...
interface BankingStore {
  ...
  /** Die abgeleitete Bewegungsliste. Am Primary wird sie bei Bedarf gerechnet; ein Rechner ohne
   *  Datenbank bekommt sie mit der Fernauskunft und liest sie von hier. */
...
export const useBankingStore = create<BankingStore>((set, get) => ({
  ...
  loadTransfers: () => {
    ...
    try {
      set(loadBankTransfersFor(localReadContext()));
    } catch { set({ transfers: [] }); }
  },
  ...
  getTransactions: () => {
    // Am Primary bleibt es bei der bisherigen Rechnung AUF ABRUF — sie kostet gut ein Dutzend
    // Abfragen und soll nicht bei jedem Laden der Uebersicht mitlaufen. Auf einem Rechner ohne
    // Datenbank ist die Liste bereits mit der Auskunft angekommen.
    if (readsFromPrimary()) return get().transactions;
    try { return bankTransactionsFor(localReadContext(), get().transfers); } catch { return []; }
  },
...
/** CENTRAL-UI-PARITY R2B — die Umbuchungen einer Filiale, zustandsfrei. */
export function loadBankTransfersFor(ctx: BusinessReadContext): { transfers: BankTransfer[] } {
  const rows = query(
    'SELECT * FROM bank_transfers WHERE branch_id = ? ORDER BY transfer_date DESC, created_at DESC',
    [ctx.branchId]
  );
...
/**
 * CENTRAL-UI-PARITY R2B — was die Bankseite zeigt: die Auskunft liefert beides auf einmal.
 *
 * Der Primary rechnet die Bewegungsliste weiterhin erst, wenn die Seite sie braucht. Fuer die
 * Ferne muss sie mitkommen, denn dort gibt es keine Datenbank, aus der man sie nachrechnen kann.
 */
export function loadBankingFor(ctx: BusinessReadContext): { transfers: BankTransfer[]; transactions: BankTransaction[] } {
  const { transfers } = loadBankTransfersFor(ctx);
  return { transfers, transactions: bankTransactionsFor(ctx, transfers) };
}

/**
 * CENTRAL-UI-PARITY R2B — die Bewegungsliste eines Kontos, als freie Funktion.
 *
 * Unveraendert in der Rechnung; sie nimmt ihre Filiale nur nicht mehr aus der Sitzung des
 * Primary, sondern aus dem Ausweis der Anfrage, und die Umbuchungen als Parameter statt aus
 * dem Zustandsspeicher.
 */
export function bankTransactionsFor(ctx: BusinessReadContext, transfers: BankTransfer[]): BankTransaction[] {
