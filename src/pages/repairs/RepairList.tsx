...
const GRID = '28px minmax(0,0.8fr) minmax(0,1fr) minmax(0,1.4fr) minmax(0,1.5fr) minmax(0,0.9fr) minmax(0,0.9fr) minmax(0,1fr) minmax(0,1fr) minmax(0,0.8fr)';
...
const REPAIR_STATUS_STYLE: Record<string, { label: string; fg: string; bg: string }> = {
  ...
  in_progress: { label: 'In Progress', fg: '#F59E0B', bg: 'rgba(245,158,11,0.10)'  },
  ready:       { label: 'Ready',       fg: '#7EAA6E', bg: 'rgba(126,170,110,0.10)' },
  picked_up:   { label: 'Picked Up',   fg: '#6B7280', bg: 'rgba(107,114,128,0.10)' },
  returned:    { label: 'Returned',    fg: '#715DE3', bg: 'rgba(113,93,227,0.10)'  },
  cancelled:   { label: 'Cancelled',   fg: '#DC2626', bg: 'rgba(220,38,38,0.10)'   },
};

const PAY_STYLE = {
  paid:    { label: 'Paid',    fg: '#7EAA6E', bg: 'rgba(126,170,110,0.10)' },
  partial: { label: 'Partial', fg: '#F59E0B', bg: 'rgba(245,158,11,0.10)'  },
  unpaid:  { label: 'Unpaid',  fg: '#DC2626', bg: 'rgba(220,38,38,0.10)'   },
  free:    { label: 'Free',    fg: '#6B7280', bg: 'rgba(107,114,128,0.10)' },
};
...
export function RepairList() {
  ...
  function getPaymentStyle(rep: Repair) {
    ...
    if (charge <= 0.005) return PAY_STYLE.free;
    if (rep.customerPaymentStatus === 'PAID') return PAY_STYLE.paid;
    if (rep.invoiceId) {
      const inv = invoices.find(i => i.id === rep.invoiceId);
      if (inv && (inv.paidAmount || 0) >= (inv.grossAmount || 0) - 0.005) return PAY_STYLE.paid;
      if (inv && (inv.paidAmount || 0) > 0.005) return PAY_STYLE.partial;
    }
    return PAY_STYLE.unpaid;
  }
  ...
  return (
    ...
    >
        ...
        <div style={{
          ...
          <div style={{ fontSize: 12, color: '#0F0F10' }}>
            ...
            {sameCustomer ? (
              ...
              <span style={{ color: '#DC2626', marginLeft: 8 }}>· all selections must be from the same client</span>
            )}
          </div>
      ...
      {/* Secondary toolbar: client filter + date sort */}
      <div className="flex items-end gap-3" style={{ marginBottom: 16 }}>
        ...
        {filterCustomerId && (
          <button onClick={() => setFilterCustomerId('')} className="cursor-pointer"
          ...
          {[{ v: 'desc' as const, label: 'Newest' }, { v: 'asc' as const, label: 'Oldest' }].map(o => (
            <button key={o.v} onClick={() => setSortDate(o.v)}
              ...
              style={{
                ...
                background: sortDate === o.v ? 'rgba(15,15,16,0.08)' : 'transparent',
                fontWeight: sortDate === o.v ? 500 : 400,
                cursor: 'pointer',
              }}>{o.label}</button>
          ))}
        </div>
      ...
      {/* Table header */}
      <div style={{ display: 'grid', gridTemplateColumns: GRID, gap: 12, padding: '0 12px 10px', alignItems: 'end' }}>
        <span />
        ...
        <span className="text-overline" style={{ textAlign: 'right' }}>COST / CHARGE</span>
      ...
      {filtered.map(rep => {
        ...
        const dateStr = fmtDate(rep.receivedAt || rep.createdAt);
        ...
        return (
          ...
          >
            ...
            <span style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}
              ...
              {eligible ? (
                <input type="checkbox" checked={checked} onChange={() => toggleSelect(rep.id)}
                  title="Select for combined invoice"
                  style={{ cursor: 'pointer', width: 15, height: 15, accentColor: '#715DE3' }} />
              ) : null}
            </span>
            ...
            <div className="flex items-center gap-2" style={{ minWidth: 0 }}>
              <div className="flex items-center justify-center rounded-full shrink-0"
                style={{ width: 28, height: 28, background: rep.repairScope === 'OWN' ? '#FFF3CD' : '#E5E9EE', border: '1px solid #D5D9DE', fontSize: 10, color: '#4B5563', fontWeight: 500 }}>
                {rep.repairScope === 'OWN' ? 'OWN' : (() => {
              ...
              <span style={{ fontSize: 13, color: '#0F0F10', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                {rep.repairScope === 'OWN' ? 'Own Stock' : getCustomerName(rep.customerId)}
              </span>
            </div>
            ...
            <div style={{ minWidth: 0 }}>
              ...
              {rep.itemReference && (
                <span className="font-mono" style={{ fontSize: 11, color: '#9CA3AF' }}>{rep.itemReference}</span>
              )}
            </div>
            ...
            <div>
              <span className="font-mono" style={{
                fontSize: 12, color: '#0F0F10', fontWeight: 600, letterSpacing: '0.06em',
                padding: '3px 8px', borderRadius: 6,
                background: 'rgba(15,15,16,0.06)', border: '1px solid rgba(15,15,16,0.12)',
            ...
            <div style={{ textAlign: 'right' }}>
              {rep.chargeToCustomer != null ? (
                ...
                <span style={{ fontSize: 12, color: '#9CA3AF' }}>{rep.estimatedCost ? `~${fmt(rep.estimatedCost)}` : '—'}</span>
              )}
            </div>
                ...
                return (
                  <button key={opt.v}
                    onClick={() => setForm({
                      ...form,
                      repairScope: opt.v,
                      customerId: opt.v === 'OWN' ? undefined : form.customerId,
                      productId: opt.v === 'CUSTOMER' ? undefined : form.productId,
                      chargeToCustomer: opt.v === 'OWN' ? undefined : form.chargeToCustomer,
                    })}
          ...
          <div>
            ...
            <textarea
              style={{
                ...
                border: 'none', borderBottomStyle: 'solid', borderBottomWidth: 1, borderBottomColor: '#D5D9DE',
                padding: '10px 0', fontSize: 14, color: '#0F0F10',
                resize: 'vertical', minHeight: 48, outline: 'none', fontFamily: 'inherit',
              }}
